 ...
 */
	function dispatch($url = null, $additionalParams = array()) {
		...
		//url 加密
		if (empty($_GET['qs']) && $_SERVER['REQUEST_METHOD'] == 'GET') 
		{ 
			$get_arr = $_GET; 
			unset($get_arr['url']); 
			if (!empty($get_arr))
			{
				$compress = bin2hex( gzcompress(http_build_query($get_arr), 9) );//base64_encode(http_build_query($get_arr))
                               
				//var_dump($compress, base64_encode(http_build_query($get_arr)), bin2hex( gzcompress(base64_encode(http_build_query($get_arr)), 9 )) );exit;
				Header("Location: " . $this->here . '?qs=' . $compress );
			}
		}
		else
		{
			if (!empty($_GET['qs']))
			{
				$uncompress = gzuncompress(pack("H*", $_GET['qs']));////base64_decode($_GET['qs']);
				parse_str($uncompress, $get_arr);
				unset($_GET['qs']);
				$_GET = array_merge($_GET, $get_arr);
				unset($_REQUEST['qs']);
				$_REQUEST = array_merge($_REQUEST, $get_arr);
				unset($this->params['url']['qs']);
				$this->params['url'] = array_merge($this->params['url'], $get_arr);
			}
		}
