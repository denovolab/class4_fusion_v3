<?php
class Complainfeedback extends AppModel{
	
	//投诉建议跟进
	var $name = 'Complainfeedback';
	var $useTable = "complain_feedback";
...
$this->query("select refill_amount, gift_amount  ,bonus_credit  from sales_strategy_charges where sales_strategy_id=$sales_strategy_id
    order by sales_strategy_charges_id");
		...
		public function findAll($currPage=1,$pageSize=10,$id,$user_type){
		//分页信息
		require_once 'MyPage.php';
		$page = new MyPage();
	 	$totalrecords = $this->query("select count(feedback_id) as c from complain_feedback  where complain_id=$id");
		...
		if (!empty($user_type)) {
			if ($user_type == 1){
				$middleSql = ",(select name from reseller where reseller_id = feed.user_id) as user_name,
				(select name from reseller where reseller_id = (select parent from reseller where reseller_id = feed.user_id)) as reseller_name
				";
			} else if ($user_type == 2) {
				$middleSql = ",(select name from client where client_id = feed.user_id) as user_name,
				(select name from reseller where reseller_id = (select reseller_id from client where client_id = feed.user_id)) as reseller_name
				";
			} else if ($user_type == 3) {
				$middleSql = ",(select card_number from card where card_id = feed.user_id) as user_name,
				(select name from reseller where reseller_id = (select reseller_id from card where card_id = feed.user_id)) as reseller_name
				";
			} 
		} 
		$sql = "select feedback_id,feed.complain_id,followed_up_on,content,feed.user_type,feed.status as fstatus,c.status  $middleSql
		  from  complain_feedback  as feed
		   left join  (select complain_id,status from  complain)c  on c.complain_id =feed.complain_id
		  where feed.complain_id=$id
order by feedback_id  	limit '$pageSize' offset '$offset'";
...
public function del($id){
		$qs = $this->query("delete from complain_feedback where feedback_id in ($id)");
		return count($qs) == 0;
}
