<?php
class Systemfunction extends AppModel{
	
	//系统功能
	var $name = 'Systemfunction';
	var $useTable = "system_function";
	var $primaryKey = "system_function_id";
...
function likequery($key,$currPage=1,$pageSize=10){
	...
	$condition="'%".$key."%'";
		...
		$sql = "select complain_id,title,substring(content from 1 for 10)  as  content,create_time ,modify_time,status from  complain  as b
			 	 where title   like $condition  or content like $condition
	 	or (select count(*)>0 from users   where user_id=b.user_id 	 	and name like $condition )
...
$this->query("select refill_amount, gift_amount  ,bonus_credit  from sales_strategy_charges where sales_strategy_id=$sales_strategy_id
    order by sales_strategy_charges_id");
		...
		require_once 'MyPage.php';
		$page = new MyPage();
	 	$totalrecords = $this->query("select count(system_function_id) as c from system_function ");
		...
		$pageSize = $page->getPageSize();
		$offset=$currPage*$pageSize;
		$sql = "select * from  system_function  
order by func_type asc   	limit '$pageSize' offset '$offset'";
