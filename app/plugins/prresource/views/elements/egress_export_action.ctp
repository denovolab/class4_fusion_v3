...
<div id="down_panel">
    <form name="myform" method="post" action="<?php echo $this->webroot ?>down/action">
    	<input type="hidden" name="type" value="egress" />
		<div id="option_panel">
			<fieldset>
				...
				<p>
					...
					<select name="fields[]">
						...
						<?php for ($j = 0; $j < $size; $j++): ?>
                                                <option <?php echo $j == $i ? 'selected' : '' ?>><?php echo $action_fields[$j]; ?></option>
						<?php endfor; ?>
					</select>
				</p>
				<?php endfor; ?>
