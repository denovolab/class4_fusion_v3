<div id="title"> 
   <h1><?php echo __('Origination', true); ?>&gt;&gt;<?php echo __('Client DID Assignment', true); ?></h1>
</div>
    ...
    $(function() {
        ...
        function put_data(data)
        {
            ...
            $.each(data, function(index, value) {
                ...
                $tr.append('<td><a href="###" title="Assign" class="put_into_cart" control="' + value[0]['number'] + '"><img src="<?php echo $this->webroot ?>images/add.png" /></a></td>');
                $tbody.append($tr);
            });
            $did_listing.show();
        }
        ...
        $put_into_cart.live('click', function(event) {
            ...
            $.ajax({
                ...
                'success': function(data) {
                    ...
                    if (data.result)
                        jQuery.jGrowl(sprintf("<?php __('The DID number [%s] is assigned successfully'); ?>", number),{theme:'jmsg-success'});
                },
                'beforeSend' : function() {
                    $loading.show();
                }
