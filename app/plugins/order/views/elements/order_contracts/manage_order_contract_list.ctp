...
<table class="list" style="margin-top:5px;">
	<thead>
		<tr>
			...
			<td><?php echo $appOrder->show_order('contract_expire_time','Expire Time')?></td>			
		</tr>
	</thead>
	<tbody>
	<?php $m = new BuyOrder?>
	...
	<tr rel="tooltip" id="contract_<?php echo array_keys_value($contract,"0.contract_id")?>">
		...
		<?php if($m->is_private($contract)):?>
			<td style="text-align:center"><?php echo $appOrder->format_time($appOrderContracts->format_order_expire_time($contract))?></td>
		<?php else:?>
