...
var selected=	$("#egress_trunks").find("option:selected");
if(selected.size()>1){
	alert("Sorry! Choose only one.");
	return;
}
var prev=$("#egress_trunks").find("option:selected").prev();
var  next=$("#egress_trunks").find("option:selected").next();

var sel_index=$("#egress_trunks").get(0).selectedIndex;
var maxIndex=$("#egress_trunks option").size() - 1;  
	if(t=='up'){
...
$(function() {
    MainSel  = document.getElementById('egress_trunks_wait');
    SlaveSel =  document.getElementById ('egress_trunks');
