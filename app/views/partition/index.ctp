...
<div id="title">
 <h1><?php  __('Management')?> &gt;&gt;<?php echo __('Partitions')?></h1>
 <ul id="title-search">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="last">
                    ...
                    <a href="javascript:void(0);" onclick="change_pwd(this,'<?php echo $item[0]['id']?>')" ><img src="<?php echo $this->webroot.'/images/menuIcon_014.gif';?>" title=" Change password"></a>
                    <!--<a title="delete" href="javascript:void(0);" onclick="del('<?php echo $item[0]['id']?>', '<?php echo $item[0]['name']?>')"><img src="<?php echo $this->webroot.'/images/delete.png'?>"></a>-->
                </td>
            </tr>
            <?php endforeach; ?>
        </tbody>
   ...
   function add(){
    ...
    function save_pwd(obj,id){
        ...
        $.ajax({
            ...
            'success':function (data){
                if(data['status'] == 'success'){
                    jQuery.jGrowl('The Partition[' + name + '] is modified successfully.',{theme:'jmsg-success'});
                    $(tr.cells[1]).html("******");
                    window.setTimeout(function() {window.location.reload(true)},3000);
                }
            }
            ...
            $.ajax({
                ...
                'success':function (data){
                    if(data['status'] == 'success'){
                        ...
                        jQuery.jGrowl('The Partition [' + name + '] can not be empty!',{theme:'jmsg-error'});
                    }else if(data['status'] == 'email_error'){
                        jQuery.jGrowl('The email field must contain a valid email address!',{theme:'jmsg-error'});
                    }else{
                        jQuery.jGrowl('The Partition [' + name + '] or Domain ['+domain+'] is already exists!',{theme:'jmsg-error'});
                    }
                }
            });
        }
