<div id="title">
    <h1><?php __('Statistics')?>&gt;&gt;<?php  __('SummaryReport')?></h1>
</div>

<div id="container">
    <ul class="tabs">
        <li <?php if($type == 1) echo 'class="active"'; ?>>
            <a href="<?php echo $this->webroot; ?>reports/summary/1">
                <img height="16" width="16" src="<?php echo $this->webroot; ?>images/list.png" />
                Origination
            </a>
        </li>
        <li <?php if($type == 2) echo 'class="active"'; ?>>
            <a href="<?php echo $this->webroot; ?>reports/summary/2">
...
<!--                <td>--><?php //echo ($arr['busy_calls'][$i] + $arr['cancel_calls'][$i] +$arr['not_zero_calls'][$i]) == 0  ? 0 :round($arr['not_zero_calls'][$i] / ($arr['busy_calls'][$i] + $arr['cancel_calls'][$i] +$arr['not_zero_calls'][$i])*100 ,2)?><!--%</td>-->
                ...
                <td><?php echo round($arr['total_calls'][$i] == 0 ? 0 : $arr['pdd'][$i] / $arr['total_calls'][$i]); ?></td>
...
<!--                <td>--><?php //echo (array_sum($arr['busy_calls']) + array_sum($arr['cancel_calls']) +array_sum($arr['not_zero_calls'])) == 0  ? 0 :round(array_sum($arr['not_zero_calls']) / (array_sum($arr['busy_calls']) + array_sum($arr['cancel_calls']) +array_sum($arr['not_zero_calls']))*100 ,2)?><!--%</td>-->
                ...
                <td><?php echo round(array_sum($arr['total_calls']) == 0 ? 0 : array_sum($arr['pdd']) / array_sum($arr['total_calls'])); ?></td>
    ...
    <?php endif; ?>
<?php echo $form->create ('Cdr', array ('type'=>'get','url' => "/reports/summary/{$type}" ,'onsubmit'=>"if($('select[name=show_type]').val() == 0) loading();"));?>
<fieldset class="query-box" style="clear:both;overflow:hidden;margin-top:10px;">
    ...
    <table class="form" style="width:100%">
        ...
        <tr>
            <td class="label">Show Inter/Intra Cost</td>
            <td class="value">
                <input type="checkbox" style="width:120px;" name="show_inter_intra" <?php if(isset($_GET['show_inter_intra'])) echo 'checked="checked"';  ?> />
            </td>
            <td class="in-out_bound label">&nbsp;</td>
            <td class="label"></td>
            <td class="value"></td>
        ...
        <tr>
            ...
            <td class="in-out_bound label">&nbsp;</td>
            <td class="label">Switch Server</td>
            <td class="value">
                <?php if (count($servers) > 1): ?>
                <?php echo $form->input('server_ip', array('options' => $servers, 'style'=>'width:120px;','empty' => '', 'label' => false, 'div' => false, 'type' => 'select')); ?>
                <?php echo $this->element('search_report/ss_clear_input_select');?>
                <?php endif; ?>
            </td>
            <td class="in-out_bound label">&nbsp;</td>
        ...
        <tr>
            <td class="label">Rate display as:</td>
            <td class="value">
                <select style="width:120px;" id="ingress_routing_plan" name="rate_display_as">
