...
$c=count($group_by_field_arr);
 ...
 echo $head_div,$table_head;
  ...
  $duration=(empty($duration)||$duration=='0')?0:$client_org[$i][0]['call_duration'];
   ...
   <td class='in-decimal'>".$client_org[$i][0]['rate_table_name'] ." </td> 
   <td class='in-decimal'>" .number_format($cdr_size, 0)." </td>
  ...
  <div class='bar'><div style='font-size:0.9em;width:".number_format($per_dis, 2)."%;'> ".number_format($per_dis, 2)." %&nbsp;</div></div></td>
   <td class='in-decimal'>" .number_format($duration/60, 0)." </td>
...
if(!empty($group_by_field_arr)){
   ...
   <td class="in-decimal">
   	<a href="<?php echo $this->webroot?>cdrreports/summary_reports?<?php echo (''==$this->params['getUrl'])?"" :($this->params['getUrl']."&code=") ?>"><?php  echo  number_format($client_org[$i][0]['cdr_size'], 0); ?> </a>&nbsp;&nbsp;(<?php echo number_format($total_per*100,2) ?>%)
   </td>
 		<!--
   ...
   <td class="in-decimal"><?php  echo  number_format($client_org[$i][0]['call_duration'], 2); ?>&nbsp;&nbsp;(<?php echo number_format($total_per1*100,2) ?>%)</td>
   <!--
...
<!--
  <tr class="totals row-1 row-2">
  	<td colspan="<?php  echo $c+1;  ?>" class="in-decimal">Total:</td>
