...
<table class="list list-form"  id="host_table">
    ...
    <tbody class="rows" id="rows-ip">
        ...
        ?>
        <tr>
            <td class="value"><input type="text" style="width: 200px;" onkeyup="value=value.replace(/[^\w\.\/]/ig,'')" name="accounts[ip][]" id="ip" value="<?php echo $ips['ip'][$i];?>"></td>
            <?php if($$hel->isIngress(@$type)){?>
        ...
        <?php $ip=split("/",$ip)?>
        <tr>
            <td class="value"><input type="text" onkeyup="value=value.replace(/[^\w\.\/]/ig,'')" style="width: 200px;" name="accounts[ip][]" class="host_ip" id="ip" value="<?php echo array_keys_value($ip,0)?>"></td>
            <?php if($$hel->isIngress(@$type)){?>
            ...
            <?php }?>
            <td class="value"><input type="text" name="accounts[port][]"  class="host_port"  id="port" maxlength="16" value="<?php echo array_keys_value($host,'0.port')?>"></td>
            <?php  if ($_SESSION['role_menu']['Routing']['gatewaygroups:view']['model_w']) {?>
            <td class="last">
                ...
                &nbsp;&nbsp;
                <a href="###" title="delete" rel="delete" class="delete_host"> <img style="margin-top:5px;" width="16" height="16" src="<?php echo $this->webroot?>images/delete.png"> </a></td><?php }?>
        </tr>
        <?php }?>
        <?php }?>
        <tr id="mb" style="">
            <td class="value"><input type="text" onkeyup="value=value.replace(/[^\w\.\/]/ig,'')" style="width: 200px;" class="host_ip" name="accounts[ip][]" id="ip"></td>
            <?php if($$hel->isIngress(@$type)){?>
            ...
            <?php }?>
            <td class="value"><input type="text" name="accounts[port][]" class="host_port" id="port" maxlength="16"></td>
            <?php  if ($_SESSION['role_menu']['Routing']['gatewaygroups:view']['model_w']) {?> 
        ...
        jQuery('.delete_host').live('click', function() {
            ...
            $.ajax({
                ...
                'success' : function(data) {
                    ...
                    jQuery.jGrowl("<?php __('Succeeded'); ?>",{theme:'jmsg-success'});
                }
            });
        });
