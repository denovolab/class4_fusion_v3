...
<?php foreach($group_by_field_arr as $field): ?>
<td><?php echo $replace_fields[$field]; ?></td>
<?php endforeach; ?>
<td width="10%" rel="0" >&nbsp;<?php echo $appCommon->show_order('release_cause_from_protocol_stack','Release Cause')?>&nbsp;</td>
<td width="10%" rel="0" >&nbsp;<?php echo $appCommon->show_order('disconnect',__('Counts',true))?> &nbsp;</td>
<td width="10%" rel="2" class="cset-1">&nbsp;<?php echo $appCommon->show_order('per',__('Counts',true))?> %&nbsp;</td>
<td width="30%" rel="2" class="cset-1">&nbsp; &nbsp;</td>

<?php if($rate_type=='org'){?>
<td width="30%" class='last' rel="2" class="cset-1">&nbsp;<?php echo $appCommon->show_order('release_cause',__('Description',true))?> &nbsp;</td>
<?php }?>
</tr>
</thead>
<tbody >
...
if(!empty($group_by_field_arr)){
$c=count($group_by_field_arr);
foreach ($group_by_field_arr as $group_by_field_key => $group_by_field_value){
$f=$group_by_field_key;
...
<td class="in-decimal"><?php echo $client_org[$i][0]['release_cause_from_protocol_stack']; ?>:<?php echo strtoupper($client_org[$i][0]['cause']);?> </td>
<td class="in-decimal">
<?php
if($rate_type=='org'){
echo $appDis->create_cdr_report_link($rate_type,$client_org[$i][0]['release_cause_from_protocol_stack'],$client_org[$i][0]['real_release_cause'],number_format($client_org[$i][0]['disconnect'], 0),$group_link_arr,explode("?",$appCommon->get_curr_url()));
}
else
{
//echo $appDis->create_cdr_report_link($rate_type,$client_org[$i][0]['release_cause_from_protocol_stack'],'',number_format($client_org[$i][0]['disconnect'], 0),$group_link_arr,explode("?",$appCommon->get_curr_url()));
echo $appDis->create_cdr_report_link($rate_type,$client_org[$i][0]['release_cause_from_protocol_stack'],$client_org[$i][0]['real_release_cause'],number_format($client_org[$i][0]['disconnect'], 0),$group_link_arr,explode("?",$appCommon->get_curr_url()));
}
?>
...
<td class="in-decimal">
<div class="bar"><div style="font-size:1.2em;width: <?php echo number_format($client_org[$i][0]['my_count']/$client_org[$i][0]['all_count']*100,3)?>% ;"><?php echo number_format($client_org[$i][0]['my_count']/$client_org[$i][0]['all_count']*100,3) ?>%&nbsp;</div></div>
