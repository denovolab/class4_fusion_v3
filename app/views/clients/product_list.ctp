<div id="title">
    <h1><?php  __('Management')?> &gt;&gt;<?php echo __('Product')?></h1>
    ...
    <table class="list" id="key_list">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td><?php echo $item[0]['rout_name']?></td>
                ...
                <td class="last">
                    ...
                    <a title="delete" href="javascript:void(0);" onclick="del('<?php echo $item[0]['id']?>', '<?php echo $item[0]['product_name']?>')"><img src="<?php echo $this->webroot.'/images/delete.png'?>"></a>
                </td>
            </tr>
            <?php endforeach; ?>
        </tbody>
...
<script>
    ...
    ?>
        rout_str += "<option value=<?php echo $value[0]['id']; ?>><?php echo $value[0]['name'];?></option>";
    <?php         
        }
    ?>
    ...
    ?>
        rate_table_str += "<option value=<?php echo $value[0]['id']; ?>><?php echo $value[0]['name'];?></option>";
    <?php         
        }
    ?>
    ...
    function add(){
       ...
       if($(".msg").length == 0 || $(".msg").css('display')=='none'){
                            $("#key_list").append("<tr>\n\
                <td><input class='in-text' type='text' name='product_name'></td>\n\
                <td><select name='rout_name' class='in-select'>"+rout_str+"</select></td>\n\
                <td><select name='rate_table_name' class='in-select'>"+rate_table_str+"</select></td>\n\
...
<a title='cancel' href='javascript:void(0);' onclick='del_add_key(this)'><img src ='<?php echo $this->webroot.'/images/delete.png';?>'></a></td>\n\
    ...
    function add_key(obj){
            ...
            var rout_id = tr.find("select[name=rout_name]").val();
            var rate_table_id = tr.find("select[name=rate_table_name]").val();
            var product_name = tr.find("input[name=product_name]").val();
            $.ajax({
                'url':"<?php echo $this->webroot.'clients/add_product';?>",
                'type':'post',
                'data':{rout_id:rout_id,rate_table_id:rate_table_id,product_name:product_name},
                'dataType':'text',
                'async':false,
                'success':function (data){
                    if(data == 'success'){
                        jQuery.jGrowl('The Product[' + product_name + '] is added successfully.!',{theme:'jmsg-success'});
                        //location = "<?php echo $this->webroot.'clients/product_list_first';?>";
                        window.setTimeout(function() {window.location.reload(true)},3000);
                    }else if(data == 'isEmpty'){
                        jQuery.jGrowl('Product Name can not be empty!',{theme:'jmsg-error'});
                    }else{
                         jQuery.jGrowl('The Product[' + product_name + '] is already exists!',{theme:'jmsg-error'});
                    }
                }
            });
    ...
    function edit_key(obj,id){
            ...
            if(edit.length == 0){
               ...
               var product_name = tr.cells[0].innerHTML;
               
               
               tr.cells[0].innerHTML = "<input type='text' class='in-text' value='"+product_name+"' name='product_name'>";
               tr.cells[1].innerHTML = "<select class='in-select' name='rout_name'>"+rout_str+"</select>";
               tr.cells[2].innerHTML = "<select class='in-select' name='rate_table_name'>"+rate_table_str+"</select>";
    ...
    function save_edit(obj,id){
            ...
            $.ajax({
                'url':"<?php echo $this->webroot.'clients/save_product';?>",
                'type':'post',
                'data':{rout_id:rout_id,rate_table_id:rate_table_id,id:id,product_name:product_name},
                'dataType':'text',
                'async':false,
                'success':function (data){
                    if(data == 'success'){
                        jQuery.jGrowl('The Product[' + product_name + '] is modified successfully.',{theme:'jmsg-success'});
                        window.setTimeout(function() {window.location.reload(true)},3000);
                    }else if(data == 'isEmpty'){
                        jQuery.jGrowl('Product Name can not be empty!',{theme:'jmsg-error'});
                    }else{
