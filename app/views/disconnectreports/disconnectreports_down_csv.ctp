...
echo $appCommon->show_order($group_by_field_arr[$i],__($group_by_field_arr[$i],true)).",";
}

}?>
"Return Code","Reason","Counts","Counts%",<?php if($rate_type=='org'){?>"Description",<?php }?>,<?php echo "\n"; ?>
<?php  
$size=count($client_org);
$total_disconnect=0;
if($size>0){
...
?>
<?php if(!empty($group_by_field_arr)){$c=count($group_by_field_arr);for ($ii=0;$ii<$c;$ii++){$f=$group_by_field_arr[$ii];$field=$client_org[$i][0][$f];$group_link_arr[$f]=$field;if(trim($field)==''){echo "Unknown";}else{echo '"'.$field.'"';}}}?>"<?php  echo  $client_org[$i][0]['release_cause_from_protocol_stack']; ?>","<?php  echo  strtoupper($client_org[$i][0]['cause']);?>","<?php if($rate_type=='org'){echo $client_org[$i][0]['real_release_cause'];}else{echo $client_org[$i][0]['real_release_cause'];}?>","<?php echo number_format($client_org[$i][0]['my_count']/$client_org[$i][0]['all_count']*100,3) ?>%",<?php if($rate_type=='org'){?>"<?php  echo  $client_org[$i][0]['release_cause']; ?>",<?php }?><?php echo "\n"; ?><?php }?><?php }}?>