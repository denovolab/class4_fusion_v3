...
<div class="container">   
	<form action="" method="POST" autocomplete='Off' id="addForm">
		<table class="cols"><col width="35%"/><col width="38%"/><col width="27%"/><tr>
			<td><!-- COLUMN 1 -->
			<fieldset>
				<legend><?php echo __('Basic Info',true);?>
      </legend>
					<table class="form">
						<tr>
					    <td style="width:25%;"><?php echo __('route_name')?>:</td>
					    <td>
						...
						<tr>
						    ...
						    <td>
						   		<?php 
						   			...
						   			echo $form->input('Dynamicroute.routing_rule',array('options'=>$arr1,'empty'=>__('select',true),'label'=>false ,'div'=>false,'type'=>'select'));
						   				?>
						    </td>
						</tr>
						<tr>
...
<?php //***************************************对接设置************************************************************?>
<fieldset>
<legend><?php echo __('alias',true);?></legend>
<table id="egress_table" class="form">
	<tr>
	    <!--<td  style="text-align: left;"></td>-->
	    <td  style="text-align: left;"><a onclick="newItem();return false;" href="#"><img src="<?php echo $this->webroot?>images/add.png"> <?php echo __('createnew',true);?></a></td>
	</tr>
	<tr>
...
<script type="text/javascript">
    jQuery(document).ready(function(){
       jQuery('#addForm').submit(function(){
          ...
          if(jQuery('#DynamicrouteRoutingRule').val()==''){
      	     jQuery('#DynamicrouteRoutingRule').addClass('invalid');
      	     jQuery.jGrowl('Routing rule cannot be NULL!',{theme:'jmsg-error'});
            return false;
                       }

          var res_id=jQuery("[name='engress_res_id[]']");
          if(res_id.length<1){
        	  res_id.addClass('invalid');
        	  jQuery.jGrowl('egress add at least one!',{theme:'jmsg-error'});
        	  return false;
              }
   ...
   jQuery('#DynamicrouteName').xkeyvalidate({type:'strName'});
    });
</script>