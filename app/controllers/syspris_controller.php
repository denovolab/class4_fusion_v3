...
//初始化查询参数
	...
	function add_syspri($module_id=null){
		...
		if (! empty ($this->data ['Syspri'] )) {
			...
			if (!empty ($return)) {
				...
				$this->Syspri->create_json_array('',201,'Add successfully');
				$this->Session->write('m',Syspri::set_validator());
				$this->redirect ( array ('controller' => 'syspris', 'action' => 'view_syspri'.'/'.$this->data ['Syspri']['module_id'] ) ); // succ
			} else {
		 ...
		 else
		  {
						if(isset($params['id']) && !empty($params['id']))
						{
							//pr($this->Syspri->find("first", Array('conditions'=>array('Syspri.id'=>$params['id']))));exit();
								$this->data = $this->Syspri->find("first", Array('conditions'=>array('Syspri.id'=>$this->params['pass'][1], 'Syspri.module_id'=>$this->params['pass'][0])));
								if(empty($this->data))
								{
	...
	function del_syspri()
	{
		...
		$this->redirect ( array ('controller' => 'syspris', 'action' => 'view_syspri'.'/'.$this->params['pass'][0] ) ); 
	}
	...
	function _create_or_update_syspri_data($params=array()) {			#update		
		...
		if(isset($params['id']) && !empty($params['id']))
		{
						   ...
						   $this->xredirect('/sysmodules/view_syspri');
							}
		}
		# add
		else
