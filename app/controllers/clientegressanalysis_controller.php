 ...
 *
 */
class ClientegressanalysisController extends AppController {
	var $name = 'clientegressanalysis';
	var $uses = array ('Cdr' );
...
//已经生成的卡  
		$sql = "select $group_by_field sum(six_seconds::integer)as total_six_seconds,sum(minutes::integer) as total_minutes,
		sum(minutes::integer)as total_bill_time,sum(cost::numeric)as total_cost
		   from   class4_view_egress_cost 
			...
			//下载
				if (	$_POST ['query'] ['output']== 'csv'){
					...
					//第一个参数是对导出的描述,第2个参数是导出的sql,第3个是导出的文件名
						$this->Cdr->export__sql_data('Export ',$org_sql,'card_cost_report');
