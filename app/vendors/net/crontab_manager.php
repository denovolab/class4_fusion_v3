...
class CrontabManager
{
	...
	function __construct($host=NULL, $port=NULL, $username=NULL, $pubkeyfile=NULL, $privkeyfile=NULL)
	{
		...
		try 
		{
			if (is_null($host) || is_null($port) || is_null($username) || is_null($pubkeyfile) || is_null($privkeyfile))
				throw new Exception("Please specify the host, port, pubkeyfile and privkeyfile!");
			...
			if (!$this->connection) 
				throw new Exception("The SSH2 connection could not be established.");
                        ...
                        $authentication = ssh2_auth_pubkey_file($this->connection, $username, $pubkeyfile, $privkeyfile);
	...
	public function append_cronjob($cron_jobs=NULL) 
	{
		...
		$this->exec($append_cronfile, $install_cron);
